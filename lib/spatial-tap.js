...
export class SpatialTap extends EventEmitter {
  constructor({ url = DEFAULT_SPATIAL_URL, reconnectMs = 1500, dryMs = Number(process.env.SPATIAL_DRY_MS ?? 30000) } = {}) {
    super();
    this.url = url;
    this.reconnectMs = reconnectMs;
    // Dry-spell watchdog: the metronome replay does not close the connection when
    // it ends — it keeps streaming EMPTY frames (people:0) indefinitely. A close-
    // triggered reconnect therefore never fires. So if no people-bearing frame
    // arrives for dryMs, force a reconnect (which, sans Last-Event-ID, restarts
    // from tipoff and brings the data back — i.e. loops the replay).
    this.dryMs = dryMs;
  ...
  async start() {
    ...
    this._watchdog = setInterval(() => {
      if (this._closed || !this.dryMs) return;
      if (Date.now() - this._lastDataAt > this.dryMs) {
        this.emit('dry', this.dryMs);
        this._lastDataAt = Date.now();            // grace before the reconnect lands
        try { this._controller?.abort(); } catch { /* ignore */ } // -> _connect re-fires from tipoff
      }
    }, Math.min(this.dryMs, 5000));
  ...
  async _connect() {
    ...
    try {
      ...
      this._lastDataAt = Date.now();  // fresh grace period for the new connection
      this.emit('open', this.url);
      await this._readStream(res.body);
    } catch (err) {
      // AbortError = intentional (stop() or the dry-spell watchdog recycling); not a fault.
      if (!this._closed && err.name !== 'AbortError') this.emit('error', err);
    }
    if (!this._closed) setTimeout(() => this._connect(), this.reconnectMs); // reconnect from tipoff (no Last-Event-ID)
  }
  ...
  async _readStream(body) {
    ...
    const dispatch = () => {
      ...
      if (summarizeFrame(payload).people > 0) this._lastDataAt = Date.now(); // liveness for the watchdog
      this.emit('frame', {
