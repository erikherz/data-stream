...
export class TsInjector extends Transform {
  constructor({ dataPid = 0x101, streamType = 0x06, maxQueue = 240,
    esDescriptor = null, wrapPayload = null } = {}) {
    super();
    this.dataPid = dataPid;
    this.streamType = streamType;
    this.maxQueue = maxQueue;
    this.esDescriptor = esDescriptor;
    this.wrapPayload = wrapPayload ?? ((raw) => raw);
    this.pmtPid = 0x1000;
    this.videoPid = 0x100;
    ...
    this.cc = 0;
    this.lastVideoPts = 0;
    this.firstVideoPts = null; // first video PTS ever seen — the tip-off anchor
  ...
  _drainInto(out) {
    while (this.pending.length) {
      const payload = this.wrapPayload(this.pending.shift());
      const pes = buildPes(payload, this.lastVideoPts);
  ...
  _transform(chunk, _enc, cb) {
    ...
    while (i + TS_PACKET_SIZE <= buf.length) {
      ...
      if (pid === this.pmtPid && pusi) {
        ...
        out.push(patchPmt(pkt, this.dataPid, this.streamType, this.esDescriptor));
        continue;
      }
      ...
      if (pid === this.videoPid && pusi) {
        ...
        if (pts != null) {
          ...
          if (this.firstVideoPts == null) this.firstVideoPts = pts;
        }
        this._drainInto(out);
      }
    }
