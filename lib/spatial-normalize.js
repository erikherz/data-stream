...
// Parse the feed's clock strings into seconds: "12:00" -> 720, "24" -> 24, "45.3" -> 45.3.
function clockSeconds(v) {
  if (v == null) return 0;
  const str = String(v);
  if (str.includes(':')) { const [m, s] = str.split(':'); return (Number(m) || 0) * 60 + (Number(s) || 0); }
  const n = Number(str);
...
export async function makeSpatialNormalizer({ game = SPATIAL_GAME } = {}) {
  ...
  function normalize(payload) {
    ...
    const frame = {
      ...
      clock: {
        period: t.period ?? 0,
        gameClockSeconds: clockSeconds(t.gameClockTime),   // "12:00" -> 720
        shotClockSeconds: clockSeconds(t.shotClockTime),   // "24"    -> 24
        running: !!t.gameClockRunning,
      },
      people,
